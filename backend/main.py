"""
ICU-HemoPredict — Backend API
XGBoost-based hemodynamic instability prediction for ICU patients.

Validated on SIUT cohort (n=253): AUROC 0.989, Sensitivity 98.4%, Specificity 95.2%
Decision threshold: 0.4774
...
import numpy as np
import pickle, shap, os, warnings
warnings.filterwarnings("ignore")

# ── Load model artifacts ───────────────────────────────────────
BASE = os.path.dirname(os.path.abspath(__file__))

with open(os.path.join(BASE, "xgb_final.pkl"), "rb") as f:
    MODEL = pickle.load(f)

FEATURE_COLS    = np.load(os.path.join(BASE, "feature_cols.npy"), allow_pickle=True).tolist()
CLINICAL_THRESH = float(np.load(os.path.join(BASE, "clinical_threshold.npy")))
EXPLAINER       = shap.TreeExplainer(MODEL)

# ── App ────────────────────────────────────────────────────────
app = FastAPI(
    title="ICU-HemoPredict API",
    description="Hemodynamic instability risk prediction · XGBoost · AUROC 0.989",
    version="1.0.0",
)
app.add_middleware(
    CORSMiddleware,
...
class PatientStayRequest(BaseModel):
    ...
    dm:         int   = Field(0,  description="Diabetes mellitus comorbidity")
    htn:        int   = Field(0,  description="Hypertension comorbidity")
    ckd:        int   = Field(0,  description="Chronic kidney disease comorbidity")
    ihd:        int   = Field(0,  description="Ischaemic heart disease comorbidity")
    copd:       int   = Field(0,  description="COPD comorbidity")
    diagnosis:  str   = Field("other", description="respiratory|renal|sepsis_shock|gi_liver|cardiac|neuro|infection_other|other")

# ── Feature engineering ────────────────────────────────────────
DIAG_CATS = ["cardiac","gi_liver","infection_other","neuro","other","renal","respiratory","sepsis_shock"]
W = 4  # rolling window = 4 slots = 8 hours

def build_feature_matrix(req: PatientStayRequest) -> tuple:
    """
    Build exact feature matrix matching training pipeline.
    Returns (X: np.ndarray shape [n_slots, 140], shap_values: np.ndarray)
    """
    readings = req.readings
    n = len(readings)

    # Pre-compute derived per-slot arrays
    maps   = [(r.sbp + 2*r.dbp)/3 for r in readings]
    shocks = [min(r.hr/max(r.sbp,1), 5) for r in readings]

    def h(attr):
        return [getattr(r, attr) for r in readings]
    ...
    for i, r in enumerate(readings):
        ...
        # Patient-level aggregates (all history up to now)
        sbp_so_far   = sbp_h[:i+1];   map_so_far = maps[:i+1]
        hr_so_far    = hr_h[:i+1];    lact_so_far= lact_h[:i+1]
        cre_so_far   = cre_h[:i+1];   fio2_so_far= fio2_h[:i+1]
        uop_so_far   = uop_h[:i+1];   rr_so_far  = rr_h[:i+1]
        avpu_so_far  = avpu_h[:i+1];  shock_so_far=shocks[:i+1]

        renal_stress = d1(cre_h) - (d1(uop_h)/50.0)
        map_def      = map_v - 65
        resp_load    = min(r.fio2/max(r.spo2,1), 2.0)

        # pp_variation: rolling std/mean of pulse pressure
        pp_hist = [readings[j].sbp - readings[j].dbp for j in range(ws, we)]
        pp_mean_w = float(np.mean(pp_hist)) if pp_hist else pp
        ...
        lookup = {
            ...
            # Time
            "time_position":    i/23.0,    "day2_flag":        int(i>=12),
            # Rolling stats
            ...
            # Composites
            "shock_index":       shock,     "spo2_fio2_ratio": sf,
            "hco3_lactate_gap":  hl,        "pp_variation":    pp_var,
            "renal_stress":      renal_stress, "map_deficit":   map_def,
            "resp_load":         resp_load,
            # Patient aggregates
            "pt_sbp_mean":  float(np.mean(sbp_so_far)),   "pt_sbp_min":  float(np.min(sbp_so_far)),
            "pt_map_mean":  float(np.mean(map_so_far)),   "pt_map_min":  float(np.min(map_so_far)),
            "pt_hr_mean":   float(np.mean(hr_so_far)),    "pt_hr_max":   float(np.max(hr_so_far)),
            "pt_lactate_max":float(np.max(lact_so_far)),  "pt_lactate_mean":float(np.mean(lact_so_far)),
            "pt_creat_max": float(np.max(cre_so_far)),    "pt_avpu_max": float(np.max(avpu_so_far)),
            "pt_fio2_max":  float(np.max(fio2_so_far)),   "pt_uop_min":  float(np.min(uop_so_far)),
            "pt_rr_max":    float(np.max(rr_so_far)),     "pt_shock_max":float(np.max(shock_so_far)),
            # Metadata
            "age":                  req.age,
        ...
        rows.append([lookup.get(col, 0.0) for col in FEATURE_COLS])
...
def risk_label(prob: float) -> dict:
    if prob >= 0.70:
        return {"level": "CRITICAL", "color": "#A32D2D",
                "message": "Critical risk — immediate haemodynamic intervention indicated"}
    elif prob >= CLINICAL_THRESH:
        return {"level": "HIGH",     "color": "#E24B4A",
                "message": "High risk — escalate monitoring, review vasopressor/fluid status"}
    elif prob >= 0.30:
        return {"level": "MODERATE", "color": "#EF9F27",
                "message": "Moderate risk — increase observation frequency"}
    else:
        return {"level": "LOW",      "color": "#639922",
                "message": "Low risk — continue standard monitoring"}


def clinical_flags(req: PatientStayRequest, probs: list) -> list:
    readings = req.readings
    maps   = [(r.sbp + 2*r.dbp)/3 for r in readings]
    sis    = [r.hr/max(r.sbp,1) for r in readings]
    flags  = []

    min_map  = min(maps);   peak_si  = max(sis)
    ...
    min_map_slot = maps.index(min_map)
    ...
    if peak_lact > 4.0:
        flags.append({"severity":"critical",
            "message": f"Lactate {peak_lact:.1f} mmol/L — severe hyperlactataemia. Tissue oxygen delivery is critically compromised. Urgent source control and resuscitation."})
    elif peak_lact > 2.0:
        flags.append({"severity":"warning",
            "message": f"Lactate {peak_lact:.1f} mmol/L — elevated above 2.0 mmol/L. Monitor trend; may indicate early tissue hypoperfusion."})
    if peak_si > 1.2:
        flags.append({"severity":"critical",
            "message": f"Shock Index {peak_si:.2f} — significant haemodynamic compromise (HR/SBP > 1.2). Consider immediate vasopressor support."})
    elif peak_si > 1.0:
        flags.append({"severity":"warning",
            "message": f"Shock Index {peak_si:.2f} — borderline haemodynamic stress. Increase cardiovascular monitoring frequency."})
    if min_sbp < 90:
        flags.append({"severity":"critical",
    ...
    if min_ph < 7.25:
        flags.append({"severity":"critical",
            "message": f"Arterial pH nadir {min_ph:.2f} — significant acidaemia. Assess metabolic vs respiratory aetiology urgently."})
    elif min_ph < 7.32:
        flags.append({"severity":"warning",
            "message": f"Arterial pH {min_ph:.2f} — mild-to-moderate acidosis. Monitor ABG trend."})
    if max_cre > 2.0:
        flags.append({"severity":"warning",
            "message": f"Creatinine peak {max_cre:.1f} mg/dL — AKI criteria met (KDIGO). Nephrology review and fluid optimisation."})
    if max_avpu >= 2:
        flags.append({"severity":"warning",
            "message": "Consciousness impaired (P/S or below) — neurological deterioration. Assess for metabolic encephalopathy, septic encephalopathy, or CNS event."})
    if min_uop < 20:
        flags.append({"severity":"warning",
            "message": f"Urine output nadir {min_uop:.0f} ml/hr — oliguria. Consider fluid challenge vs diuretic resistance."})
    if len(probs) >= 4 and probs[-1] > probs[0] + 0.15:
        flags.append({"severity":"warning",
            "message": "Progressive risk increase across observation window — haemodynamic trajectory is deteriorating. Reassess current management plan."})
    return flags
...
def root():
    return {
        ...
        "version": "1.0.0",
        "model": "XGBoost",
        "validation_auroc": 0.9891,
        "sensitivity": 0.9838,
        "specificity": 0.9524,
        "decision_threshold": round(CLINICAL_THRESH, 4),
        "cohort": "SIUT Karachi, n=253 patients",
    }

@app.get("/health")
def health():
    return {"status": "ok", "model_loaded": MODEL is not None, "n_features": len(FEATURE_COLS)}

@app.post("/predict/stay")
def predict_stay(req: PatientStayRequest):
    try:
        ...
        for i, prob in enumerate(probs):
            ...
            top_features = [
                {"feature": FEATURE_COLS[j], "shap": round(float(sv[j]), 4),
        ...
        high_slots = sum(1 for p in probs if p >= CLINICAL_THRESH)
        trend      = ("deteriorating" if probs[-1] > probs[0] + 0.10
                      else "improving" if probs[-1] < probs[0] - 0.10
                      else "stable")

        # SHAP mean |shap| over full stay for top features
        ...
        return {
            "summary": {
                ...
                "trend":              trend,
                "decision_threshold": round(CLINICAL_THRESH * 100, 1),
            },
            "slots":          slots,
